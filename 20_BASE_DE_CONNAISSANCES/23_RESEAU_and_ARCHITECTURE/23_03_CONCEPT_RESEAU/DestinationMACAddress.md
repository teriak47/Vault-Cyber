...
*   **Action** : La couche liaison de données encapsule le paquet IP dans une trame Ethernet. L'adresse MAC de destination est insérée au début de l'en-tête Ethernet, suivie de l'adresse MAC source et du type EtherType. Le commutateur lit cette adresse MAC de destination pour déterminer sur quel port il doit transférer la trame. Il utilise sa table d'adresses MAC (CAM table) pour faire correspondre l'adresse MAC de destination avec un port de sortie. Si l'adresse n'est pas trouvée, la trame est diffusée sur tous les ports, sauf celui d'où elle provient (flooding).
