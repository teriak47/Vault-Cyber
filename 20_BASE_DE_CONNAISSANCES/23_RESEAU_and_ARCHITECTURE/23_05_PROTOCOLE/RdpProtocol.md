...
1.  **Négociation de la capacité (Capability Exchange)** : Le client RDP initie une connexion TCP au port 3389 du serveur. Une fois la connexion TCP établie, le client et le serveur échangent des "[[ProtocolDataUnit|PDU]] de négociation" pour déterminer les capacités supportées par chacun, telles que la version du protocole, les capacités graphiques, et les méthodes de chiffrement.
