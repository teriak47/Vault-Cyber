...
*   **Attaques par Déni de Service (DoS/DDoS)** : Les **attaques par inondation UDP (UDP Flood)** sont courantes. Un attaquant envoie un grand volume de datagrammes UDP à des ports aléatoires sur un serveur cible, souvent avec des adresses IP sources usurpées. Le serveur est alors contraint de générer de nombreuses réponses ICMP "Destination Unreachable", épuisant ses ressources et entraînant un déni de service pour le trafic légitime. Une autre vulnérabilité récente est le *Loop Denial-of-Service* (CVE-2024-2169) où des serveurs peuvent communiquer indéfiniment, conduisant à un DoS.
