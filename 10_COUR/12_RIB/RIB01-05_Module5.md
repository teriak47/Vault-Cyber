...
> 3. Comparer et mettre en correspondance les **modèles [[TcpIpModel|TCP/IP]] et [[OsiModel|OSI]]**, en identifiant les fonctions de chaque couche.
...
*   **Codage** : Conversion des messages en [[BinaryDigit|bits]], puis en diverses formes physiques (sons, ondes lumineuses, impulsions électriques).
...
> [!note] Définition Clé
> **RFC - Request for Comments** : Chaque étape du processus de développement et d'approbation d'une norme est enregistrée dans un document RFC numéroté, publié et géré par l'[[InternetEngineeringTaskForce|IETF]] (Internet Engineering Task Force).

Les protocoles réseau reflètent souvent les **protocoles de communication humaine**, impliquant un langage commun, des règles de comportement et des mécanismes de confirmation. L'étude de protocoles comme *Ethernet*, *[[InternetProtocol|IP]]*, *[[TransmissionControlProtocol|TCP]]* et *[[HttpProtocol|HTTP]]* est fondamentale pour comprendre le fonctionnement, la configuration et le dépannage des réseaux.
...
Le modèle **[[TcpIpModel|TCP/IP]]** (Transmission Control Protocol/Internet Protocol) est l'architecture fondamentale de l'[[Internet]], développée au début des années 70. Il définit quatre catégories de fonctions essentielles pour la communication.

> [!note] Les Quatre Couches du Modèle TCP/IP
> 1.  **[[ApplicationLayer|Application]]** : Représente les données pour l'utilisateur, inclut le codage et le contrôle du dialogue. C'est l'interface directe avec les logiciels (ex: [[Browser|navigateurs web]], clients email).
...
> 3.  **[[NetworkLayerOSI|Internet]]** : Détermine le meilleur chemin à travers le réseau. Responsable de l'adressage logique et du [[RoutingConcepts|routage]] des paquets de données.
...
> 2.  **Présentation ([[PresentationLayer|Couche 6]])** : Assure la représentation commune des données transférées (chiffrement, compression, conversion).
> 3.  **Session ([[OsiSessionLayer|Couche 5]])** : Organise le dialogue et gère l'échange de données entre applications (établissement, maintien, terminaison des connexions).
> 4.  **Transport ([[TransportLayer|Couche 4]])** : Définit les services pour segmenter, transférer et réassembler les données entre périphériques finaux (fiabilité, [[FlowControl|contrôle de flux]]).
> 5.  **Réseau ([[NetworkLayerOSI|Couche 3]])** : Fournit des services pour échanger des parties de données sur le réseau entre périphériques finaux (adressage logique, routage).
> 6.  **Liaison de Données ([[DataLinkLayer|Couche 2]])** : Décrit les méthodes d'échange de trames de données entre périphériques sur un support commun (adressage physique, détection d'erreurs).
> 7.  **Physique ([[PhysicalLayer|Couche 1]])** : Décrit les moyens mécaniques, électriques, fonctionnels et méthodologiques pour activer, gérer et désactiver les connexions physiques pour la transmission de bits.
...
*   La **[[TcpIpApplicationLayer|Couche Application TCP/IP]]** englobe les couches Application (7), Présentation (6) et Session (5) du modèle OSI.
*   La **[[TransportLayerTCPIP|Couche Transport TCP/IP]]** correspond à la couche Transport (4) du modèle OSI.
*   La **[[InternetLayerTCPIP|Couche Internet TCP/IP]]** correspond à la couche Réseau (3) du modèle OSI.
*   La **[[NetworkAccessLayerTCPIP|couche Accès Réseau TCP/IP]]** regroupe les couches Liaison de Données (2) et Physique (1) du modèle OSI.
