...
Lorsque vous utilisez l'adressage [[InternetProtocolVersion4|IPv4]] statique, il est important de tenir à jour une liste qui permet de savoir précisément quelles [[InternetProtocol|adresses IPv4]] ont été attribuées à quels appareils.

## Adressage Dynamique (DHCP)

Les [[InternetProtocol|adresses IPv4]] peuvent être attribuées automatiquement à l'aide d'un [[Protocol|protocole]] appelé [[DynamicHostConfigurationProtocol|DHCP]] (Protocole de Configuration d'Hôte Dynamique). 
Le [[DynamicHostConfigurationProtocol|protocole DHCP]] est généralement la méthode d'attribution d'[[InternetProtocol|adresses IPv4]] privilégiée pour les réseaux de grande taille, car il dégage le personnel de support de cette tâche et le risque d'erreur de saisie est presque éliminé.
