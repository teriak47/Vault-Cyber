...
Parfois, un [[Host|hôte]] doit envoyer un [[Message|message]], mais il ne connaît que l'[[InternetProtocolAddress|adresse IP]] du [[NetworkDevice|périphérique]] de destination. L'[[Host|hôte]] doit connaître l'[[MediaAccessControlAddress|adresse MAC]] de ce [[NetworkDevice|périphérique]]. 
...
*   **[[InternetProtocolAddress|Adresse logique]] (l'[[InternetProtocolAddress|adresse IP]])** - Utilisée pour envoyer les [[Packet|paquets]] depuis le [[NetworkDevice|périphérique]] source vers le [[NetworkDevice|périphérique]] de destination. L'[[InternetProtocolAddress|adresse IP]] de destination peut se trouver soit sur le même [[Network|réseau]] IP que la source soit sur un [[RemoteNetwork|réseau distant]].
